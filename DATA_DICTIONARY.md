# Data Dictionary — PI3KAtlas

**Dataset:** PI3K–AKT–mTOR pathway gene alterations across the TCGA PanCancer Atlas (32 tumour types, n = 10,195).
**Source:** cBioPortal public REST API (v7.0.0, retrieved June 2026). **Genes (15):** PIK3CA, PIK3R1, PIK3CB, PIK3R2, PTEN, AKT1, AKT2, AKT3, MTOR, TSC1, TSC2, RICTOR, RPTOR, STK11, INPP4B.
...
## 1. `tcga_pancan_pi3k_akt_mtor_alteration_freqs_long.csv` — master long table (480 rows × 23 cols)
...
15 gene rows × 33 columns = `geneSymbol` index + 32 tumour-type columns; each cell is the frequency for that gene in that tumour (any-alteration / CNA-only / mutation-only respectively).

## 5. `pi3k_akt_mtor_gene_rank_summary.csv` (15 rows × 9 cols)
...
## 6. `pi3k_akt_mtor_top5_tumours_per_gene.csv` (75 rows × 9 cols)
`geneSymbol`, `rank` (1–5), `tumour_acronym`, `N`, `any_alter_freq`, `mut_freq`, `any_cna_freq`, `amp_freq`, `deep_del_freq`.

## 7. `pi3k_akt_mtor_tumour_burden_ranking.csv` (32 rows × 7 cols)
`rank` (1–32), `tumour_acronym`, `mean_any_alter_freq`, `mean_mut_freq`, `mean_any_cna_freq` (means across all 15 genes), `N`, `n_genes` (=15). Ranked by `mean_any_alter_freq` (desc).

## 8. `brca_pi3k_akt_mtor_alteration_frequencies.csv` (15 rows × 15 cols; N = 1052)
...
## 10. `tcga_low_coverage_tumours_for_pi3k_akt_mtor.txt`
